package br.com.tln.personalcard.credenciador.moshi
...
class TelenetBaseResponseJsonAdapter<T>(val moshi: Moshi, resultsType: Type) : JsonAdapter<TelenetBaseResponse<T>>() {
        ...
        while (reader.hasNext()) {
            when (reader.selectName(options)) {
                0 -> status = reader.nextInt()
                    ?: throw JsonDataException("Non-null value 'status' was null at ${reader.path}")
                1 -> results = tNullableResultsAdapter.fromJson(reader)
                    ?: throw JsonDataException("Non-null value 'results' was null at ${reader.path}")
                -1 -> {
